...
def conversion(inp,out,check):
	if os.path.isdir(inp):
		...
		for entry in sorted(os.listdir(inp)):
			conversion(os.path.join(inp, entry),os.path.join(out, entry),os.path.join(check, entry))
		...
		try:
			...
			try:
				...
				print("wrote " + write)
				if (data != open(os.path.splitext(check)[0]+extension, 'rb').read()):
...
try:
	...
	out = os.path.join(input("Check directory:"),os.path.basename(inp))
except:
	inp = "jsons/"
	out = "rtons/"
	check = "check/"
